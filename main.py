...
def write_output_json(objects_tracked, images, predictions, filename = 'result.json'):
    ...
    with open(filename, 'w') as file:
        ...
        json.dump(final_output_dict, file, indent = 4)
@torch.no_grad()
def run(
        source='0',
            ...
            if det is not None and len(det):
                ...
                if len(outputs[i]) > 0:
                    for j, (output) in enumerate(outputs[i]):
                        ...
                        if save_vid or save_crop or show_vid or save_json:  # Add bbox to image
                            ...
                            aggregate_bbox = [bbox_left, bbox_top, bbox_w, bbox_h]
                            tracked_object_dict = {
                                "id": c,
                                "supercategory": names[c]
                            }

                            objects_tracked.append(tracked_object_dict)
                            prediction_dict = {
                                "id": prediction_id,
                                "image_id": image_id,
                                "predicted_object_id": int(c),
                                "bbox": aggregate_bbox,
                                "track_id": int(id),
    ...
    objects_tracked_dup_rem = pd.DataFrame(objects_tracked).drop_duplicates().to_dict('records')
    write_output_json(objects_tracked_dup_rem, images, predictions)
    # Print results
